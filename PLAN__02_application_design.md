...
Start with one target pace, such as five seconds with half-second variation. Try other paces in separate trials. Hold the item order, browser version, viewport, cache policy, seed, discovery limits, and network type constant where possible. Repeat a useful condition, ideally three times initially, and report the individual results and range. This is a practical comparison, not a statistical guarantee.

Compare Cloudflare configurations using the same recorded conditions and repeat after the expected cooldown or on another documented exit. Changing IP also changes a condition; do not combine results from unlike networks into a single threshold. A short manually operated browser session can later help assess how well the scripted workflow represents researcher behavior.
...
| `CF_SETTINGS_LABEL`, `CF_SETTINGS_NOTES` | Required user-supplied reference and description; allow `unknown` explicitly, with a report limitation. Include settings effective time. |
...
- `events.jsonl`: incremental navigation, request, response/failure, readiness, timing, and interference records using elapsed time, UTC time, and internal IDs. Flush regularly so interruption preserves useful evidence.
- `summary.md`: readable conditions, achieved pace, first interference evidence, the four trailing windows, reached checkpoints, limitations, and space for Central IT's later finding. JSON supplies machine-readable counts; no database or separate spreadsheet export is needed initially.
...
| **Implement evidence and reports** | Observe document/subresource outcomes, stop at first relevant signal, retain partial event records, record conditions, and calculate all four windows plus reached checkpoints. | Central IT log integration, optional reviewed screenshots/traces, report comparison UI, richer challenge recovery analysis. |
...
Meaningful initial tests should cover configuration precedence and invalid input; reproducible bounded intervals; slow-page overruns without catch-up bursts; duplicate and paginated item links; collection exhaustion; challenge/denial/network-error distinctions; a challenged subresource; redirect counting; exact time-window boundaries and partial coverage; and preservation of results on timeout or interruption. Use saved/synthetic events and a local fixture server, not repeated production traffic, for automated tests. Run `uv run ./run_tests.py` according to repository instructions.

The initial implementation is complete when an operator can supply a collection and settings reference, watch sequential browsing at a recorded varying pace, obtain an honest stop reason and the four requested window summaries, and start an independent trial after changing exits externally. Source-code implementation and live measurements await a later user request.
